...
export default function Home() {
  ...
  async function analyzeFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const workbook = XLSX.read(await file.arrayBuffer(), { type: "array" }); const counts = { EMPLOYER: 0, CUST: 0, EXPENSES: 0, USERS: 0 } as Record<LoadKey, number>;
      (Object.keys(loads) as LoadKey[]).forEach((key) => { const sheet = workbook.SheetNames.find((name) => loads[key].aliases.includes(norm(name))); if (sheet) counts[key] = XLSX.utils.sheet_to_json<unknown[]>(workbook.Sheets[sheet], { header: 1, defval: "" }).slice(1).filter((row) => row.some((cell) => String(cell).trim())).length; });
      const userSheet = workbook.SheetNames.find((name) => loads.USERS.aliases.includes(norm(name)));
      if (!userSheet) throw new Error("Aba de Colaboradores não encontrada.");
      ...
      setLoadRows(counts); setUsers(data.filter((row) => row.some((cell) => String(cell).trim())).map((row, index) => ({ row: index + 2, name: valueAt(row, indexes.name), cpf: valueAt(row, indexes.cpf), email: valueAt(row, indexes.email), sexo: valueAt(row, indexes.sexo), ativo: valueAt(row, indexes.ativo) }))); setHistory([]);
