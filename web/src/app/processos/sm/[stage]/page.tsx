...
import { smStages, getSmStage } from "@/data/sm-process";
...
export default async function SmStagePage({ params }: { params: Promise<{ stage: string }> }) {
  ...
  const stage = getSmStage(stageId);
  if (!stage) notFound();

  const index = smStages.findIndex((s) => s.id === stage.id);
  const previous = smStages[index - 1];
  const next = smStages[index + 1];

  return (
    <>
      <nav className="ci-breadcrumb"><Link href="/processos">Central de Implantação</Link> {"> "}<Link href="/processos/sm">Small &amp; Medium</Link></nav>
      <h1 style={{ color: "var(--heading)", fontSize: 26, marginBottom: 4 }}>Processo de Implantação — Small &amp; Medium</h1>
      <p style={{ marginBottom: 20 }}>Do recebimento ao go live, tudo o que você precisa em um só lugar.</p>

      <Timeline stages={smStages} activeId={stage.id} basePath="/processos/sm" />

      <div className="ci-card" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
        <div>
          <span className="ci-owner-badge ci-owner-consultor" style={{ marginBottom: 8, display: "inline-block" }}>{stage.order}</span>
          <h2 style={{ margin: "4px 0" }}>{stage.title}</h2>
          <p style={{ margin: 0 }}>{stage.shortDescription}</p>
        </div>
        <div style={{ display: "flex", gap: 10, flexShrink: 0 }}>
          {previous ? <Link href={`/processos/sm/${previous.id}`} className="ci-nav-link" style={{ color: "var(--heading)", border: "1px solid var(--button-border)" }}>← Etapa anterior</Link> : <span />}
          {next && <Link href={`/processos/sm/${next.id}`} className="ci-nav-link" style={{ background: "var(--yellow)", color: "var(--navy-deep)", fontWeight: 800 }}>Próxima etapa →</Link>}
        </div>
      </div>
